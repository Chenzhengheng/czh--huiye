...
```text
React / TypeScript 前端
  ├─ 日记、图片、思考线、回响反馈：按 ChatGPT 账号保存到 Sites R2
  ├─ 未保存草稿与少量界面设置：当前设备的 localStorage
  ├─ 写作、日记池、详情、Markdown、完整备份与导入
  └─ 调用 Worker API

Cloudflare Worker
  ├─ /api/data：读取和保存当前登录账号的完整私人数据
...
密钥只在服务端环境变量中使用。日记原文保存在 Sites 私人对象存储；只有用户允许参与未来回响的记录会在发起关联判断时发送给模型。数据接口只接受平台转发的已登录 ChatGPT 用户身份，不信任客户端自行声明的邮箱。
...
- 日记、图片、思考线和回响反馈按 ChatGPT 账号保存在私人云端；未保存草稿与少量界面设置保存在当前浏览器。
- Markdown 导出用于阅读与归档；完整 JSON 备份与导入用于换电脑迁移。
- 完整 JSON 包含思考、图片、关联许可、思考线、回响和回响检查记录；未保存草稿不在备份内。
- JSON 导入会替换当前账号云端保存的日记与回响记录，导入前应确认当前数据是否需要另行保留。
