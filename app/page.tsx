...
export default function Home(){
  ...
  return <main className="app-shell">
    ...
    {selected&&edit&&<div className="modal-back"><div className="review" style={{width:"min(680px,100%)",maxHeight:"92vh",overflow:"auto"}}><div className="review-head"><div><span className="spark">□</span><div><small>{selected.date} · {selected.source}</small><h2>查看与编辑日记</h2></div></div><button onClick={()=>{setSelectedId(null);setEdit(null);}}>×</button></div><div className="review-body"><label>标题</label><input value={edit.title} onChange={e=>setEdit({...edit,title:e.target.value})}/><label>正文</label><textarea style={{minHeight:220}} value={edit.content} onChange={e=>setEdit({...edit,content:e.target.value})}/><label>标签</label><TagEditor tags={edit.tags} onChange={tags=>setEdit({...edit,tags})}/><Toggle checked={edit.aiLink} onChange={()=>setEdit({...edit,aiLink:!edit.aiLink})} label="允许 AI 关联" hint="关闭后，这篇记录不会参与未来召回"/>{selected.attachments?.length?<div style={{display:"flex",gap:10,overflowX:"auto",marginTop:10}}>{selected.attachments.map((a,i)=><img key={i} src={a.data} alt={a.name} style={{width:120,height:90,objectFit:"cover",borderRadius:7}}/>)}</div>:null}</div><div className="review-note">{selected.originalContent&&selected.originalContent!==selected.content?"原文版本仍被单独保留。":"当前内容就是原始版本。"}</div><div className="review-actions" style={{justifyContent:"space-between"}}><button onClick={()=>download([selected],selected.title)}>导出本篇</button><button className="primary" onClick={saveEdit}>保存修改</button></div></div></div>}
