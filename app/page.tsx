...
export default function Home(){
  ...
  const writeLineCount=visualLineCount(text);
  const writeRows=Math.min(15,Math.max(6,writeLineCount+3));
  const editLineCount=visualLineCount(edit?.content||"",55);
  const editRows=Math.min(15,Math.max(6,editLineCount+3));
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="mobile-head"><div className="brand"><span className="brand-mark">回</span><span>回页</span></div><button onClick={()=>setView("pool")}>日记池</button></header>
      {view==="write"&&<div className="page write-page" style={{maxWidth:960,paddingTop:44,transform:`translateY(-${Math.min(190,Math.max(0,writeLineCount-5)*20)}px)`,transition:"transform .28s ease"}}><div className="eyebrow">2026 年 7 月 12 日 · 星期日</div><h1>此刻，想留下什么？</h1><p className="lead">不用想标题，也不用急着归类。先写下来就好。</p><div className="paper" style={{height:writeRows*41+(writeLineCount<15?58:100),overflowY:"hidden",transition:"height .28s ease"}}><textarea style={{paddingBottom:writeLineCount>=15?72:0,overflowY:writeLineCount>=15?"auto":"hidden"}} value={text} onChange={e=>setText(e.target.value)} placeholder="一个疑问、一段推理，或只是此刻不想忘记的感受……" autoFocus/><div className="paper-tools"><span>{text.length} 字 · {attachments.length} 张图片</span><div><button onClick={pasteText}>粘贴</button><button onClick={()=>fileRef.current?.click()}>＋ 手写 / 图片</button><input ref={fileRef} hidden type="file" accept="image/*" multiple onChange={e=>{addFiles(e.target.files);e.target.value="";}}/></div></div></div>
