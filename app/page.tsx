...
type OrganizationExample = { id: number; original: string; title: string; content: string; tags: string[]; prompt: string; createdAt: string };
...
export default function Home() {
  ...
  useEffect(() => { if (examplesReady) localStorage.setItem(EXAMPLES_KEY, JSON.stringify(organizationExamples)); }, [organizationExamples, examplesReady]);  useEffect(() => { const timer = window.setInterval(() => setNow(Date.now()), 30_000); return () => window.clearInterval(timer); }, []);
  ...
  function saveOrganizationExample() {
    if (!reviewOriginal.trim() || !review.content.trim()) return;
    const example: OrganizationExample = { id: Date.now(), original: reviewOriginal, title: review.title, content: review.content, tags: review.tags, prompt: organizePrompt, createdAt: new Date().toISOString() };
    setOrganizationExamples(current => current.some(item => item.original === example.original && item.content === example.content) ? current : [example, ...current].slice(0, 30));
    notify("已收为好样例，用于之后校准整理规则");
  }
