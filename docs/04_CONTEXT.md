...
**AnonymousPortfolioVisit（作品集匿名访问）**：一台非管理员设备请求某一 PublicPortfolioDeployment 的公开作品集入口，并成功获得页面响应所形成的匿名统计单位。它不再依赖浏览器 Beacon 区分已确认与未确认；同一设备在同一部署内 30 分钟重复打开或刷新只计一次，先后访问国内主站与海外备份则各记一次。Cookie 去重只是尽力而为：禁用或清除 Cookie 可能导致重复计数，系统不使用 IP 或浏览器指纹补偿。可明确识别的爬虫、链接预览和预加载请求不计入；无法可靠识别时接受少量误差。不记录姓名、公司或账户身份。避免：把页面请求数称为用户数、继续展示已确认／未确认或确认率、统计 PortfolioMode 演示页、把管理员检查计入访问、为去重引入更强的访客识别。

**PortfolioAdminDevice（作品集管理员设备）**：经专属授权标记、可读取私有访问看板且其公开作品集访问不进入访客统计的设备。管理员标记依赖每个 PublicPortfolioDeployment 各自的浏览器 Cookie：已有的海外备份标记继续有效，国内主站只需单独标记一次；清除 Cookie 或更换浏览器后需对对应部署重新标记。避免：公开看板数据、仅依靠可猜测路径保护看板、试图跨域共享管理员 Cookie、把管理员刷新计入匿名访问。

**PortfolioDeploymentSource（作品集部署来源）**：一次 AnonymousPortfolioVisit 所属的 PublicPortfolioDeployment，当前只区分国内主站 `huiye-ai.cn` 与海外备份 `chatgpt.site`。它用于分站汇总和排查采集异常，不是访客身份或地理位置。避免：根据来源推断访客所在地、用部署来源跨域合并访客。

**LocalPortfolioVisitDashboard（本地作品集访问看板）**：只在负责人本地电脑打开的私有统计界面，通过管理员授权只读获取公开作品集的匿名访问汇总。看板以两个 PublicPortfolioDeployment 的合计为主，同时按 PortfolioDeploymentSource 保留分站数据，便于识别单站采集异常。它不是作品集网站的一条公开或私有展示路由；线上作品集只承载不可见的访问上报与受保护统计接口。避免：只显示合计而隐藏来源异常、把看板页面部署进作品集、让普通访客获得统计入口、把本地看板误称为作品集功能。

**PortfolioVisitSession（作品集访问会话）**：同一匿名浏览器设备在同一 PublicPortfolioDeployment 内、30 分钟窗口中对公开作品集入口的一次或多次成功响应。PortfolioMode 演示页不参与统计。避免：把刷新次数当作独立访客、继续用页面 Beacon 划分确认状态、把两个公开部署的会话强行合并、从历史日志倒推上线前数据。
